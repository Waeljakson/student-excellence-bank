...
function mustReplace(search, replacement, label) {
  if (!src.includes(search)) throw new Error(`student-import-autodetect: marker not found: ${label}`);
...
function inferDepartmentKind(wb: XLSX.WorkBook, fileName: string): "secondary" | "middle" | "" {
  ...
  // ملفات المدرسة الحالية تضع بيانات المدرسة/المرحلة في الشيت الأول.
