...
type Tab = "dashboard" | "checks" | "khameesna" | "students" | "rankings" | "rewards" | "admin" | "system" | "account" | "referrals" | "behavioral" | "redemption" | "student-evaluations" | "followup" | "periodic-evaluations" | "teacher-stats";
...
function AppShell({ profile, children, tab, setTab }: { profile: Profile; children: any; tab: Tab; setTab:(t:Tab)=>void }) {
  ...
  const canViewTeacherAchievement=profile.roles?.some(r=>["TEACHER","SUPER_ADMIN","SCHOOL_ADMIN","PRINCIPAL","VICE_PRINCIPAL","GUIDANCE_COUNSELOR"].includes(r));
...
function AdminView({pending,users,staff,classes,reload,isSuperAdmin}:{pending:PendingUser[];users:ManagedUser[];staff:StaffMember[];classes:AdminClass[];reload:()=>Promise<void>;isSuperAdmin:boolean}){
  ...
  async function changeStaffName(s:StaffMember){
